...
\verb|trait|-ova (veoma slična funkcionalnost u poredjenju sa interfejsom u drugim jezicima). 
...
U odnosu na vrednost tipa \verb|kind|, stavke mogu ili ne mogu da poseduju atribute. Vidljivost je 
još jedan bitan aspekt svake stavke.

Prilikom poziva parsiranja, jedini fajl koji se odmah procesira jeste glavni \verb|main| fajl.
Makroi i drugi moduli se parsiraju pomoću procesa ekspanzije. 
...
Ekspanzija je proces tokom kog se dopunjuje apstraktno sintaksno stablo. 
Svi makroi su prošireni (odatle ekspanzija) na regularni izvorni kod. 
Na mestima gde se koriste moduli, izvorni kod modula je umetnut (\verb|inlined|).
Deo AST stabla će postati samo funkcije drugih modula koje su implicitno ili eskplicitno korišćene 
...
u većinskom delu rustc kompajlera. Ovo je reprezentacija apstraktnog sintaksnog stabla 
nakon parsiranja, ekpanzije makroa, rezolucije imena tj. procesa snižavanja (\verb|lowering|).
Tokom procesa snižavanja, neke forme izražavanja bivaju posjednostavljene (\verb|desugared|).
Na primer, for petlje se ne pojavljuju u HIR-u jer su konvertovane u loop.

Glavni čvor HIR-a se isto kao i kod AST-a naziva \verb|Crate| ali ima drugačije podatke,
i sadrži veliki broj mapa koje služe da omoguće lakši pristup sadržaju.

HIR poseduje značajan broj identifikatora:
- DefId - referencira definiciju unutar bilo kog \verb|Crate|-a.
- LocalDefId - referencira definiciju unutar trenutno kompajliranog \verb|Crate|-a.
- HirId - referencira bilo koji čvor u HIR-u.
